...
from .icmppo.curiosity.base import Curiosity, CuriosityFactory
...
class CNNICMModel(ICMModel):
    def __init__(self, state_converter: Converter, action_converter: Converter):
        ...
        # print('state converter shape', state_converter.shape)
        with torch.no_grad():
            n_flatten = self.cnn(
                # torch.as_tensor(state_converter).float()
                torch.rand(size=(1,) + state_converter.shape).float()
            ).shape[1]
        # print('n_fatten', n_flatten)
