...
hp_settings = {
    ...
    'non-stationarity_modulo': [1e6],  # toggle settings to induce non-stationarity at this interval
}
env_configs = {
    'cfg1': [{
...
experiment_name = 'DQN_continuous_1e6_' + env_name + '/forcemag20-5'  # used for creating log and save paths
...
def run_training_instance(args):
    ...
    env = gym.make(env_name, param_dict=[env_cfg['cfg1'], env_cfg['cfg2']],
                   change_interval=hp_dict['non-stationarity_modulo'], interpolate=True)
    env = InfoWrapper(env)
    env = SoftHorizonWrapper(env, soft_horizon=hp_dict['soft_horizon'], max_resets=1)

    print('creating model...')
    model = DQN('MlpPolicy', env, verbose=1, batch_size=hp_dict['policy_batch_size'],
